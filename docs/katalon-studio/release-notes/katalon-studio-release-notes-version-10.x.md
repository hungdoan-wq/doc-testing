...
:::note
- The in-app upgrade from version 9.x to 10.x is not available. To download the latest version 10.x, go to Katalon website: [Download Katalon Studio](https://katalon.com/download/).
...
This release delivers key enhancements to StudioAssist, MCP Server tools, and Windows app testing. StudioAssist introduces new AI execution options in Katalon Runtime Engine, and automatic retry handling in both Ask and Agent modes. MCP Server tools offer expanded support for variables and test object types. Windows app testing includes additional element properties. The release also provides extensive fixes across StudioAssist, test execution, web UI, mobile testing, integrations, and overall platform performance and stability.

### Enhancements

- [StudioAssist]
    - Enabled AI failure reason analysis for for HTML and email reports generated by KRE (Katalon Runtime Engine). See [Command-line syntax in Katalon Runtime Engine](/katalon-studio/execute-tests/katalon-runtime-engine/command-line-syntax-in-katalon-runtime-engine).
    - Enabled **automatic retries** for operations when Agent/Ask mode encountered execution errors.
    - You can now attach images to StudioAssist chat to provide context for more accurate test generation and troubleshooting. This applies to both Ask and Agent modes. See [Attach images and screenshots](/katalon-studio/studioassist/studioassist-agent-mode#attach-images-and-screenshots).
    - [MCP Server tools]
        - Added support for variables in `Get test case` and `Create/update test suite`. The `Get test case` tool can retrieve variables in a test case, and `Create/update test suite` can automatically fetches those variables when adding that test case to a test suite. See [Katalon built-in MCP servers](/katalon-studio/studioassist/mcp-servers/katalons-built-in-mcp-servers#katalon-studio-mcp-server).
        - Improved `createTestObject` and `updateTestObject` by replacing them with new tools for each AUT type, for more accurate results and better performance. New available tools include:
            - `createWebTestObject`
    ...
    - [Window app testing] Added support for additional element properties such as `@IsEnabled` so they can be used in **XPath** during Windows application testing.
    - [KRE] Added new params to support overriding multiple ADO test plans.

### Changes
- Automated code migration recipes are now available only for paid KSE users. Free and education users will see a paywall dialog when attempting to use this feature.
- [KRE] Removed the `licenseRelease` command-line option and its suggestion in the KRE CLI builder, as the underlying mechanism is no longer valid. 

### Fixes

- [StudioAssist]
    - Fixed an issue where StudioAssist could not create a test object using the **Gemini** provider **(Flash/Pro)** when the **instructionFormatTool** file is missing.
    - Fixed an issue where StudioAssist could not create a test object using the **Test Object** tool when using the **AWS Bedrock** provider.
    - Fixed an issue where StudioAssist responded with a `400 Bad Request` error when using **OpenAI or Azure OpenAI** models to enter text containing accented (non-ASCII) characters, such as “á”, “ắ”, “ê”, “ộ”, or “ü”, enabling proper support for multilingual inputs (e.g., Vietnamese, French, and German).
    - Fixed an issue where StudioAssist showed both checkboxes for **Append tags for test cases used AI capabilities** as selected on first launch, even though only one option should be selectable.
- [Execution]
    - Fixed an issue where the **Run From Here** option was disabled after a test script failed.
    - Fixed an issue (since 10.4.1) where **Self-healing** did not activate to find web elements using alternative locators, despite the Self-healing setting being enabled.
    - Fixed an issue where navigating to keywords from scripts takes longer time.
    ...
    - Fixed an issue where importing a **OpenAI/Swagger .json file** causes a Java heap space error even with increased heap size.
- [Web UI]
    - Fixed an issue where `WebUI.uploadFile` uploads previous files again when executed multiple times in sequence.
    - Fixed an issue where `WebUI.callTestCase` did not automatically add required import statements for objects and factories.
    - Authenticate keyword failed on Chrome 143 beta and later versions.
    - [Self-healing] Excluded keyword list caused self-healing to fail completely for multi-nested shadow DOM elements.
- [Security]
    - Fixed an issue where login credentials were not saved after logging in with **Remember me** selected.(org.eclipse.equinox.security.storage.StorageException: No password provided).
    - Fixed a login issue with a specific account where the login dialog remains after successful login.
- [Mobile testing]
    - Fixed an issue where the locator value in the **Attributes locator strategy** for **Mobile Test Objects** did not update in real time after including, removing, or updating **Object Properties**.
    - Fixed an issue where editing and saving a **Mobile Test Object** using the **Save** or **Save All** buttons did not persist changes in **Katalon Studio 10.4.2**.
- [Desktop testing] XPath expressions with additional properties like `@IsEnabled='True'` not supported during Windows app testing.
- [Desired Capabilities] Fixed an issue where the **profile.default_content_setting_values.notifications** preference was not working in Katalon Studio 10.x due to changes in Selenium 4.
- [Kobiton Integration]
    - Fixed an issue where Katalon Studio 10.3.1 could not connect to Kobiton On-Premise.
    - Fixed an issue where Katalon Studio 10.4.1 could not execute with Kobiton devices, with error message: "Missing required information for Kobiton Execution".
- StudioAssist did not use Max Completion Token setting.
- [Wait logic] Fixed an issue where `Wait For Element Clickable` is marked as passed even though the element is not clickable.
- [KRE] Fixed an issue where a global variable defined as Boolean in profile is cast to String when passed via `-g_` argument in KRE 10.2.x+.
- [Recorder Plus] Fixed an issue where **Recorder Plus** cannot record event `SendKey` "**Enter**".
- Web Recorder onboarding tour did not work on Chrome & Edge due to local network restrictions.

### Known limitation

- [StudioAssist | MCP Server tools] The **httpBodyContent** is currently not supported when creating or editing REST and SOAP test objects.
...
- [StudioAssist | Agent mode]
    - Added 3 new MCP tools to extend StudioAssist’s automation capabilities, see [Katalon built-in MCP servers](/katalon-studio/studioassist/mcp-servers/katalons-built-in-mcp-servers):
        - **Test Object**
...
- [Reporting | HTML Report]
    - Added **Expand All/Collapse All** button and enhanced **Search** functions. See [View test suite and test suite collection reports in Katalon Studio](/katalon-studio/test-reports/view-test-reports/view-test-suite-and-test-suite-collection-reports-in-katalon-studio#apply-new-report-template-and-split-report).
    - Added shortcut links in the Test Suite Collection (TSC) HTML report to directly access related Test Suite reports.
- [API testing] Added a **Copy as cURL** option and cURL commands in API request logs and reports (applies to REST/SOAP) to support API debugging. See [View and customize execution log in Katalon Studio](/katalon-studio/test-reports/view-test-reports/view-and-customize-execution-log-in-katalon-studio).
...
- Introduced Automated Code Migration (Beta) tool to automatically update and fix your project's code for seamless major version upgrades (for example, from 8.x or 9.x to 10.x). This tool helps resolve breaking changes in BDD, Selenium, and dependencies. See [Quick Guide for Auto Code Migration in Katalon Studio 10.4.0 (Beta)](/katalon-studio/get-started/workspace-settings/quick-guide-for-auto-code-migration-tool).
- [Reporting] Added `${tsc_case_result_table}` to include all test cases and their statuses from all test suites in a Test Suite Collection, and `${tsc_case_result_table_minimal}` to include only failed or error test cases. See [Share test reports via email in Katalon Studio](/katalon-studio/test-reports/share-test-reports-via-email-in-katalon-studio#customize-email-template-for-test-suite-collection).
- [StudioAssist] 
  - Introduced AI Test Case Failure Analysis (not yet supported in KRE) to automatically analyze and summarize test failures using AI at both test case and test suite levels. The AI insights are displayed in **HTML** and **email reports.** See [AI Failure Analysis in Katalon Studio](/katalon-studio/studioassist/studioassist-ai-failure-analysis).
  - Expanded StudioAssist’s AI provider options with AWS Bedrock compatibility. See [StudioAssist Preferences](/katalon-studio/studioassist/studioassist-preferences).
  - Added **Agent instruction** and **AI failure analysis** to the StudioAssist Prompt Library for prompt customization. See [Customize engineering prompts with Prompt Library](/katalon-studio/studioassist/studioassist-preferences#customize-engineering-prompts-with-prompt-library).
  - Expanded StudioAssist MCP Servers:
    - Added 13 new MCP tools to extend StudioAssist’s automation capabilities. These tools let you programmatically create, update, and retrieve project assets such as test suites, custom keywords, BDD files, and test data. See [Katalon's built-in MCP servers and tools](/katalon-studio/studioassist/mcp-servers/katalons-built-in-mcp-servers) for detailed descriptions and examples. 
  ...
  - StudioAssist now supports HTTP-based MCP servers which requires authentication (for example, Atlassian). See [Supported MCP Servers in StudioAssist](/katalon-studio/studioassist/mcp-servers/mcp-servers-and-tools-in-studioassist#supported-mcp-servers-in-studioassist). 
  - Supported adding **Atlassian SSE MCP Server** to integrate with StudioAssist Agent mode. See [Adding an external MPC Server](/katalon-studio/studioassist/mcp-servers/mcp-servers-and-tools-in-studioassist#add-an-external-mcp-server). 
  - Added **TestOps MCP Server** (enabled by default) for seamless connection and data exchange. See [TestOps MCP Servers](/katalon-platform/testops-mcp-server).
- [Onboarding] Revamped Onboarding Tour for Test Suite with an in-app tour introduces new users to running, and analyzing test suites. See [In-app Onboarding Tours with Katalon Studio](/katalon-studio/get-started/in-app-onboarding-tours-with-katalon-studio). 
- [Desktop App Testing] Officially supported Desktop App Testing (via FlaUI driver) with improved performance and element handling. See [Desktop app testing with FlaUI driver in Katalon Studion](/katalon-studio/manage-projects/set-up-projects/windows-desktop-apps-testing/desktop-applications-testing-with-flaui-driver-in-katalon-studio).
    - Added `Select Option By Label`  keyword for Windows using FlaUI. See [[Windows] Select Option By Label](/katalon-studio/keywords/keyword-description-in-katalon-studio/windows-keywords/windows-select-option-by-label).
    - Added  `Windows.mouseOver` keyword to the Windows keyword library, allowing users to execute mouse-over (hover) actions on Windows application elements. See [[Windows] Mouse Over](/katalon-studio/keywords/keyword-description-in-katalon-studio/windows-keywords/windows-mouse-over).
    - Added support for the WinAppDriver legacy capability `ms:waitForAppLaunch` to better handle slow-launching desktop applications, preventing immediate failures during the `Windows.switchToWindowTitle` keyword execution. See [Desired Capabilitites](/katalon-studio/manage-projects/set-up-projects/windows-desktop-apps-testing/desktop-applications-testing-with-flaui-driver-in-katalon-studio#desired-capabilities).
    - Supported more element attributes in Windows Spy/Recorder.
- [API Testing] Introduced API Collection which supports authentication inheritance at the folder level, reducing repetitive configuration, improving consistency, and simplifying credential management. See [API Collection for bulk managing authorization](/katalon-studio/test-objects/api-test-objects/authorization/api-collection-for-bulk-managing-authorization).

### Enhancements

- Added Chrome 141, Edge 141, and Firefox 144 compatibilities.
- [Wait logic] Added a new toggle in Project Settings to enable or disable the enhanced waiting and checking mechanism. See [Wait and Enhanced Waiting & Checking Keyword Logic](/katalon-studio/keywords/using-keywords-in-katalon-studio/web-testing/wait-and-enhanced-waiting-&-checking-keyword-logic). 
- [Execution] Enhanced execution settings where users can configure per-platform (Web, Mobile, Windows) timeouts and per-keyword delays for more granular control. See [Execution settings in Katalon Studio](/katalon-studio/manage-projects/project-settings/execution-settings-in-katalon-studio).
- [Mobile Testing] 
...
- [StudioAssist]
  - Introduced **Agent mode**, a real-time AI collaborator in StudioAssist. Agent mode interprets natural-language prompts, breaks them into multi-step actions, and keeps iterating until tasks are complete. With Agent mode, you can:
    ...
    - Generate better code and troubleshooting steps, with knowledge leveraged from [Katalon Docs](/katalon-studio/about-katalon-studio) site.
    - Connect to external HTTP-based MCP servers (no authentication supported yet).
    - Learn more in [StudioAssist Agent mode](/katalon-studio/studioassist/studioassist-agent-mode).
  - Added 2 Katalon MCP servers by default, an open standard that allows AI to connect with tools and project data:
    ...
    - Learn more at [Katalon built-in MCP servers](/katalon-studio/studioassist/mcp-servers/katalons-built-in-mcp-servers).
...
- [Mobile Testing] Implemented support for new scroll to text mobile keyword functionality. See [Enhanced Scroll To Text](/katalon-studio/keywords/keyword-description-in-katalon-studio/mobile-keywords/mobile-enhanced-scroll-to-text).
- [Web Testing] Added new `WebUI.jsClick` keyword for JavaScript-based click operations. See [[WebUI] JavaScript Click](/katalon-studio/keywords/keyword-description-in-katalon-studio/web-ui-keywords/webui-jsclick#example).
- Added a re-login button that uses saved login credentials when a forced logout occurs. For details, see the note under [Enable Remember Me in Login](/katalon-studio/get-started/activate-licenses#enable-remember-me-in-login).
- Implemented a new **Remember Me** option in login dialog with auto-session refresh and retry logic. See [Enable Remember Me in Login](/katalon-studio/get-started/activate-licenses#enable-remember-me-in-login).
...
- [Reporting]
    ...
    - Enhanced TestRail integration with support for mapping multiple TestRail IDs. See [Map test cases between Katalon Studio and TestRail](/katalon-studio/integrations/test-management/configure-testrail-integration-in-katalon-studio#map-test-cases-between-katalon-studio-and-testrail).
...
- Added project trust dialog when opening untrusted KS projects for enhanced security. See [Trust dialog on first open](/katalon-studio/manage-projects/manage-test-projects/open-a-test-project-in-katalon-studio#trust-dialog-on-first-open).
...
- [API Testing]
    - Introduced a new option to save API response bodies as files. Includes a **Save Response Body as File** link in the **Response** tab. See [RESTful endpoint in object repository](/katalon-studio/create-test-cases/create-your-first-api-test-with-katalon-studio#restful-endpoint-in-object-repository).
...
- [Mobile Testing]
    - Added support for Self-Healing in Mobile Testing to improve resilience for mobile test scripts. See [Configure Self-Healing Tests](/katalon-studio/maintain-tests/self-healing-tests-in-katalon-studio#configure-classic-self-healing).
...
- Revamped the Onboarding Experience and added a Knowledge Hub to simplify how users get started, see more in [Knowledge Hub](/katalon-studio/get-started/workspace-settings/katalon-help#knowledge-hub):
...
- Introduced the ability to automatically update Azure DevOps (ADO) Test Cases upon test execution by setting the Automation status to `Automated` and adding the executed test case name in the **Associated Automation** section. This eliminates manual updates and improves visibility of automated test coverage in ADO. See [Enable Azure DevOps Integration in Katalon Studio](/katalon-studio/integrations/test-management/configure-azure-devops-test-plans-integration-in-katalon-studio).
- [Reporting]
    - Introduced a new test suite and test suite collection email report template with a modern UI look and more data displayed. See [Customize email template for test suite](/katalon-studio/test-reports/share-test-reports-via-email-in-katalon-studio#customize-email-template-for-test-suite).
...
- [StudioAssist]
    - Now supports Google Gemini integration using both personal keys and a shared organization key set by an admin, and fully compatible with the `gemini-2.5-flash` model. See [StudioAssist Preferences](/katalon-studio/studioassist/studioassist-preferences).
...
- Introduced a new setting in Web Recorder Plus that allows user to configure ignoring pattern when generating locator. See [Configure exclusion patterns from locator generation](/katalon-studio/maintain-tests/self-healing-tests-in-katalon-studio#exclude-keywords).
...
- Improved Smart Wait to Reduce Test Flakiness and Improve Execution Speed. See [Smart Wait function](/katalon-studio/record-and-spy/webui-record-and-spy-utilities/smart-wait-function).
...
- Katalon Studio’s Web Recorder [failed to launch on Chrome version 137](https://katalon-inc.my.site.com/support/article/Recorder-Fails-to-Launch-on-Chrome-137) due to changes in browser automation protocols, including the removal of the `--load-extension` flag. 
  ...
  :::note
  For detailed steps on setting up your desired capabilities, see: [Set up Desired Capabilities for WebUI Testing in Katalon Studio](/katalon-studio/manage-projects/project-settings/desired-capabilities/set-up-desired-capabilities-for-webui-testing-in-katalon-studio)
  :::
...
Introduced StudioAssist chat window and improved StudioAssist accuracy by upgrading to the GPT 4o-mini model. You can now instantly access and chat with StudioAssist from within your Katalon Studio app. See [StudioAssist Ask mode](/katalon-studio/studioassist/studioassist-ask-mode).
