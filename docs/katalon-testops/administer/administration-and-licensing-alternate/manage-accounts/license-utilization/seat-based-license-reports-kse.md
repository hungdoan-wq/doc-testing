...
You can use it to:
- Review active and historical sessions, including user, email, session ID, machine ID, license type, start time, end time, last active time, session state, and duration.
- Search and filter sessions by user, session ID, machine ID, or date range.
- Monitor session states (Active, Inactive, Ended) to understand current license consumption and identify sessions approaching automatic termination.
- Track session activity patterns and identify long-running sessions with the updated duration calculation that reflects actual active usage time.
...
Sessions move through different states based on activity:
- **Active**: The session is currently in use and receiving activity signals.
- **Inactive**: No activity has been detected for 5 minutes. The session remains open and can return to Active if activity resumes within 7 days.
- **Ended**: The session has been closed, either explicitly by the user, automatically after 7 days from the start time, or by system enforcement.

### Understanding Duration Calculation
Duration represents the total time a session was actively used, calculated differently based on the session state:

- Session **Active/ Inactive** state: Duration = Last Active Used - Started (continuously updates as activity occurs)
- Session **Ended** state: Duration = Ended - Started (finalized and will not change)

This ensures duration accurately reflects actual active usage time rather than total elapsed time.

### Report Columns
- **Session ID**: A unique identifier for each KSE session. Use this to track specific sessions across filters and exports.
- **Machine ID**: A unique identifier for the device where the session is running. This helps you identify which machines are consuming licenses.
- **Started**: The date and time when the session started (DD MMM YYYY HH:mm:ss, displayed in your timezone)
- **Ended**: The date and time when the session ended (DD MMM YYYY HH:mm:ss, displayed in your timezone). Empty (—) for Active or Inactive sessions.
- **Duration**: Total time the session was actively used (formats: 45s, 5m, 15m 30s, 1h 05m, 1h 05m 15s)
- **Last Active Used**: The last time, the system has recorded, this session was actively used (DD MMM YYYY HH:mm:ss, displayed in your timezone)
- **State**: Current status of the session: Active, Inactive, or Ended

**Note:** Sessions are automatically ended after 7 days from the Started time to prevent indefinite license consumption.

<img src="https://tw-cdn.katalon.com/katalon-platform/admin/license-ultilization/kse-user-session-jan20.png" alt="Seat-Based: KSE - User Session" width="1080"/>
